...
class Usuario
{
    ...
    public function registrar($usuario, $senha)
    {
        ...
        if (count($resul) > 0) {
            ...
            if (count($resul) > 0) {
                ...
                if (count($resul) > 0) {
                    //chamar função que adicionara o ponto de ontem, passando usuario e dia anterior
                    $this->registrarRecursivo($id_usuario, date('Y-m-d', strtotime('-1 days')));
                }
    ...
    private function registrarRecursivo($id_usuario, $data)
    {
        ...
        if (count($resul) > 0) {
            ...
            if($this->BuscarDataEmArray($data, $resul)){
                ...
                $this->registroVazio($id_usuario, $data, $meta);
                //chamar registrarRecursivo passando a variavel data - 1 dia
                $this->registrarRecursivo($id_usuario, date('Y-m-d', strtotime($data.'-1 days')));
            }
    ...
    private function registroVazio($id_usuario, $data, $meta){
        $query = "INSERT INTO folha (id_usuario, data, saldo) VALUES (:id_usuario, :data, :saldo)";

        $saldo = $meta * 60 * 60;
        //converter segundos para horas no formato 00:00:00
        $saldo = gmdate("H:i:s", $saldo);
    ...
    private function BuscarDataEmArray($data, $array)
    {
        foreach ($array as $value) {
            // echo("Verificando se ".$value['data']." é igual a $data");
            if ($value['data'] == $data) {
                return true;
            }
        }
